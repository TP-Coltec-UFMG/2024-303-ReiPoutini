...
public class Kurrea : MonoBehaviour {
    ...
    void AtualizarDirecaoEColisor() {
        if (destinoAtual == pontoA) {
            sprites.flipX = true;
            Ataque.offset = new Vector2(-3.3f, -0.2f);
            Detectar.offset = new Vector2(-14.5f, 0.2f);
        } else {
            sprites.flipX = false;
            Ataque.offset = new Vector2(-1.6f, -0.2f);
            Detectar.offset = new Vector2(-10.0f, 0.2f);
        }
    }
